...
foreach zone in  inter region balance sub  {
	...
	foreach case in con uncon  {
		foreach thehour in $hoursAll{
			...
			* only 60589 stays because it doesn't have a "source" for one year
			...
			if "`case'"=="con"{
				merge 1:1 idnum using "data/hourly22/coefsconstrained_`zone'`thehour'_uni_allinone.dta", nogen keep(2 3)
			}
			if "`case'"=="uncon"{
			...
			* put residual coal and residual gas in with gas and coal
			...
			if "`zone'"=="region"{
				if "`case'"=="con"{
					collapse (sum) btildademand*, by (Fuel)
					foreach reg in CAL CAR CENT FLA MIDA MIDW NE NW NY SE SW TEN TEX{
						replace btildademand`reg' = 0 if abs(btildademand`reg') < 0.00001
					}
				}
				if "`case'"=="uncon"{
	...
	* end foreach case
	...
	dfasf
save "data/coefs_fuel_`zone'22_uni.dta", replace
* end foreach zone
}
