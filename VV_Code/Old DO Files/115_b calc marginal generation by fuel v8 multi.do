*Same as Andy's file, just saving to tempdir so as not to erase previous results
*** marginal generation by fuel for each of inter, region, balance, sub

*local hours_list $hoursAll
local hours_list 23
		*inter region balance sub
foreach zone in  region  {
	local count_iter = 1
	
	foreach case in con uncon  {
		foreach thehour in `hours_list'{
			...
			* only 60589 stays because it doesn't have a "source" for one year
			...
			if "`case'"=="con"{
				merge 1:m idnum using "${tempdir}/coefsconstrained_`zone'`thehour'.dta", nogen keep(2 3)
			}
			if "`case'"=="uncon"{
				merge 1:m idnum using "${tempdir}/coefs_`zone'`thehour'.dta", nogen keep(2 3)
			}
...
save "${tempdir}/coefs_fuel_`zone'22.dta", replace
* end foreach zone
}
*end
