...
use "data/hourly22/coefsconstrained_`level'`thehour'_uni.dta", clear
merge 1:1 idnum using "data/hourly22/coefsconstrained_`level'`thehour'_uni_allinone.dta",nogen keep(3)
merge 1:1 idnum using "data/hourly22/plant_unit_to_idnum_crosswalk`thehour'.dta",nogen keep(3)
...
if "`level'"=="sub" {
	matrix rsq = J(121,1,.)
	global codes  $AllsubBAcodes
	
}
if "`level'"=="balance" {
	matrix rsq = J(55,1,.)
	global codes  $AllBAcodes
}
if "`level'"=="region" {
	matrix rsq = J(13,1,.)
	global codes  $AllRegions	
}

if "`level'"=="inter" {
	matrix rsq = J(3,1,.)
	global codes  Texas West East
}
local cc = 1
foreach var in $codes{
...
local cc = 1
foreach var in $codes{
	qui replace name = "demand`var'" in `cc'
	local cc = `cc'+1
}
merge 1:1 name using $temp1


scatter rsq1 size, xtitle("Std of demand") ytitle("Rsq of uni and uni_all_in_one") title("Hour `thehour' Level `level'") graphregion(color(white))  name(gr`level')
...
*graph combine grsub grbalance grregion grinter

graph combine grregion grinter

*graph export "latex22/compare_all_in_one`thehour'.png", replace
