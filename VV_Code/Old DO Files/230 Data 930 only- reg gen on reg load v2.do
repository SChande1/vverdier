...
if "$`region2'_imports" =="can" keep utcdate utchour demand gencoal gengas gennuke genoil genwater gensun genwind genother sumng can   year month $`region2'_imports
if "$`region2'_imports" =="mex" keep utcdate utchour demand gencoal gengas gennuke genoil genwater gensun genwind genother sumng mex  year month $`region2'_imports
if "$`region2'_imports" =="" keep utcdate utchour demand gencoal gengas gennuke genoil genwater gensun genwind genother sumng   year month $`region2'_imports
...
if "$`region2'_imports" =="can"  rename can gencan`region2'
if "$`region2'_imports" =="mex" rename mex genmex`region2'
...
use $temp2, clear
gen sumngCAN = gencanNY + gencanNW + gencanNE + gencanMIDW + gencanCENT
gen sumngMEX = genmexTEX + genmexCAL
save $temp2, replace
...
foreach generate in $AllRegions CAN MEX {
	...
	*use $temp1, clear
	...
	qui gen constant = 1
		if "`generate'"== "CAN" | "`generate'"== "MEX" local varstouse sumng`generate' $AllDemands constant  i.group
		else local varstouse sumng`generate' $AllDemands constant wnd`generate' ghi`generate' i.group
...
replace region="CAN" in 14
replace region="MEX" in 15
...
foreach generate in $AllRegions {
	...
	foreach type in coal gas nuke oil water sun wind other $`generate'_imports {
...
foreach region in $AllRegions{
	foreach type in coal gas nuke oil water sun wind other $`region'_imports {
